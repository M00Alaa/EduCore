  ...
  private formDataValue(value: any): any {
   ...
   private fromDataObjectToFormData(parentKey: string, arr: any[], formData?: FormData): void {
    ...
    arr.forEach((arrItem, index) => {
      if (arrItem) {
        if (arrItem instanceof Blob) {
          formData?.append(`${parentKey}`, this.formDataValue(arrItem));
        } else if (Array.isArray(arrItem)) {
          // Recursively handle nested arrays
          this.fromDataObjectToFormData(`${parentKey}[${index}]`, arrItem, formData);
        } else if (arrItem instanceof Object) {
          Object.entries(arrItem).forEach(([key, val]) => {
            if (Array.isArray(val)) {
              // Recursively handle nested arrays
              this.fromDataObjectToFormData(`${parentKey}[${index}].${key}`, val, formData);
            } else if (val instanceof Blob) {
              formData?.append(`${parentKey}[${index}].${key}`, this.formDataValue(val));
            } else if (val instanceof Object) {
              const toAppend = this.formDataValue(val);
              if (toAppend !== null) {
                formData?.append(`${parentKey}[${index}].${key}`, toAppend);
              }
            } else {
          ...
          formData?.append(`${parentKey}[${index}]`, this.formDataValue(arrItem));
        }
      }
    });
  }
